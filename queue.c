...
void add(struct Linked_list* list, int value) {
    // reference a prev se è la prima metti NULL
...
void delete(struct Linked_list* list, int pos) {
    ...
    while(node->next != NULL && index < pos) { // scorre fino al nodo da eliminare
        ...
        prev = node;
        node = node->next;
        next = node->next; // quello dopo ancora
        index++;
    };
...
void dequeu(struct Linked_list* list) {
	list->tail = list->tail->prev;
	free(list->tail);
};

void enqueue(struct Linked_list* list,int value) {
	struct Node* node = (struct Node*) malloc(sizeof(struct Node));
	node->value = value;
	// node->next = NULL;
	node->prev = NULL;
